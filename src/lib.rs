...
impl<T> Tree<T> where T: PartialOrd + Debug {
    ...
    pub fn with_capacity(n: usize) -> Tree<T> {
        Tree {
    ...
    pub fn predecessor(&self, value: T) -> Option<&T> {
        ...
        let predecessor_node = self.predecessor_node(node);
        if predecessor_node.is_null() {
            return None;
        } else {
            return Some(&self[predecessor_node].value);
        }
    }

    pub fn successor(&self, value: T) -> Option<&T> {
        let node = self.find_node(value);
        let successor_node = self.successor_node(node);
        if successor_node.is_null() {
            return None;
        } else {
            return Some(&self[successor_node].value);
        }
    }
    ...
    pub fn insert(&mut self, value: T) {
        if self.root.is_null() {
            ...
            let ptr = self.insert_at(value, self.root);
            self.insert_repair(ptr);
        }
    }

    pub fn delete(&mut self, value: T) {
        let pointer = self.find_node(value);

        self.delete_node(pointer);
    }

    fn delete_node(&mut self, at: Pointer) -> Option<T> {
        ...
        if !self[at].left.is_null() && !self[at].right.is_null() {
            ...
            let predecessor = self.predecessor_node(at);
            self.swap(predecessor, at);
            return self.delete_node(at);
    ...
    fn predecessor_node(&self, at: Pointer) -> Pointer {
        if at.is_null() {
            return Pointer::null();
        }
    ...
    fn successor_node(&self, at: Pointer) -> Pointer {
        if at.is_null() {
            return Pointer::null();
        }
    ...
    fn find_node(&self, value: T) -> Pointer {
        fn find_node_at<T>(tree: &Tree<T>, value: T, at: Pointer) -> Pointer where T: PartialOrd + Debug {
            ...
            if tree[at].value == value {
                return at;
            }

            if value < tree[at].value {
                find_node_at(tree, value, tree[at].left)
            } else {
                find_node_at(tree, value, tree[at].right)
            }
        }

        find_node_at(self, value, self.root)
    }
    ...
    fn insert_at(&mut self, value: T, at: Pointer) -> Pointer {
        ...
        // Precondition: at is not a null pointer
        if at.is_null() {
            panic!("Error inserting node: null pointer");
        }

        if value.lt(&self[at].value) {
            // Insert left
            if self[at].left.is_null() {
                self[at].left = Pointer(self.nodes.insert(create_node(value, at)));
                return self[at].left;
            } else {
                return self.insert_at(value, self[at].left);
            }
        } else {
            // Insert right
            if self[at].right.is_null() {
                self[at].right = Pointer(self.nodes.insert(create_node(value, at)));
                return self[at].right;
            } else {
                return self.insert_at(value, self[at].right);
            }
        }
    }
